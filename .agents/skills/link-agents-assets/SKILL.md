...
| --- | --- | --- |
| `repo` | リポジトリルート | `AGENTS.md`, `.claude/skills`, `.claude/agents` |
...
| `github` | `.github` ディレクトリ | `skills/<skill-name>`(`--type skills` の明示指定時のみ), `agents/<agent-name>.agent.md`, `copilot-instructions.md`(`--type instructions` の明示指定時のみ) |
